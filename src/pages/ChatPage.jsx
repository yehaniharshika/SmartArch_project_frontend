...
export default function ChatPage() {
  ...
  const { plan, loading: planLoading, error: planError } = usePlanByToken(token);
  ...
  useEffect(() => {
    if (plan && !welcomedRef.current) {
      ...
      const name = plan.project_name || plan.original_filename || "your floor plan";
      dispatch(
        addBotMessage(
  ...
  return (
    ...
    // Only the messages panel below (and the sidebar) scroll internally.
      ...
      <div className="flex-1 min-h-0 max-w-[1600px] w-full mx-auto flex gap-6 px-4 sm:px-6 py-6 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col gap-4 min-w-0 min-h-0">
          ...
          {/* Messages Area — THE ONLY scrollable region in the chat panel.
              min-h-0 is required here: without it, this flex child refuses
              to shrink below its content height, so overflow-y-auto never
              kicks in and the whole page scrolls instead.
              no-scrollbar hides the scrollbar visually while keeping the
              area fully scrollable (mouse wheel, touch, keyboard). */}
          ...
          {rooms.length > 0 && (
            <div className="card p-4 space-y-3 flex-shrink-0">
              ...
              <div className="space-y-1">
                {rooms.map((room, i) => (
                  <div key={i} className="flex items-center gap-2 py-1 border-b border-stone-100 last:border-0">
